...
def clean_isbn(isbn):
    ...
    # Use ISBN10 if available, otherwise use ISBN13
...
def normalize_books(input_file = 'books.csv'):
    '''
    ...
    try:
        with open(input_file, 'r', encoding = 'utf-8') as f:
            ...
            for row in reader:
                ...
                if not isbn:
                    print("Warning: Skipping book with missing ISBN10: " + row.get('Title', 'Unknown'))
                    continue
                
                # Ensure ISBN is exactly 10 characters (pad with leading zeros if needed)
                isbn = isbn.zfill(10)[:10]
                
                title = row.get('Title', '').strip()
                if not title:
                    print("Warning: Book with ISBN " + isbn + " has no title")
                    continue
                
                # Normalize title (capitalize first letter of each word)
                title = ' '.join(word.capitalize() if word.lower() not in ['a', 'an', 'the', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with'] 
                               or i == 0 
                               else word.lower() 
                ...
                if not authors:
                    print("Warning: Book '" + title + "' has no authors listed")
                ...
                for author_name in authors:
                    ...
                    # Add to book_authors relationship
                    book_authors.append({
                        'Author_id': authors_dict[author_name],
                        'Isbn': isbn
                    })
        
        print("Processed " + str(len(books)) + " books with " + str(len(authors_dict)) + " unique authors")
    ...
    print("Created book.csv")
    
    # Write authors.csv
    authors_list = [{'Author_id': aid, 'Name': name} for name, aid in authors_dict.items()]
...
def normalize_borrowers(input_file='borrowers.csv'):
    '''
    ...
    try:
        with open(input_file, 'r', encoding = 'utf-8') as f:
            ...
            for row in reader:
                ...
                # Combine first and last name into Bname
        ...
        print("Processed " + str(len(borrowers)) + " borrowers")
